...
class TonClient(object):
    def __init__(self):
        ...
        self.needTotalChapter = 5  # 需要创建章节的数量

    def prepare(self, client, userinfo):
        self.httpClient = client
        self.userinfo = userinfo
        self.ton_headers = LocustPublic.ton_headers(self.userinfo.get('token'))
        location = self.userinfo.get('location')
        self.landIndex = location.get('landIndex')
        self.adventureId = location.get('adventureId')
        self.round = location.get('round')
        if self.adventureId == 0:
            self.blockUser = 0
            self.tokenType = 1
